...
export default function AssignmentDetailTeacherPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const assignments = (await assignmentsRes.json()) as AssignmentApi[] | null;
  ...
  async function handleGradeSubmit() {
    ...
    try {
      const res = await fetch("/api/submissions", {
        ...
        body: JSON.stringify({ id: gradeTarget.id, grade: gradeValue, feedback: feedbackValue }),
      });
      ...
      await fetch("/api/notifications", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ title: "Assignment graded", message: `You received ${gradeValue} on ${assignmentTitle}`, recipientId: gradeTarget.studentId }),
      });
          ...
          <div className="relative bg-white rounded-lg p-6 z-10 w-full max-w-md">
            ...
            <div className="space-y-3">
              <div>
                ...
                <input value={gradeValue} onChange={(e) => setGradeValue(e.target.value)} className="w-full mt-1 p-2 border rounded" />
              </div>
              <div>
              ...
              <div className="flex justify-end gap-2">
                ...
                <button onClick={handleGradeSubmit} disabled={grading} className="rounded-xl bg-[#1d6d58] px-4 py-2 text-white font-semibold">
